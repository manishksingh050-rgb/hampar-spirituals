...
export default function CheckoutPage() {
  ...
  const orderId = 'HSP-' + Math.random().toString(36).substr(2,8).toUpperCase()
  ...
  const s = {
    ...
    progressInner: { display:'flex', alignItems:'center', maxWidth:600, margin:'0 auto', gap:0 },
    progStep: (active, done) => ({ display:'flex', alignItems:'center', gap:8, flex:1 }),
    ...
    progLabel: (active, done) => ({ fontSize:'0.75rem', letterSpacing:'1px', textTransform:'uppercase', color: active||done ? '#2a2a1e' : '#9a9a7a' }),
    progLine: (done) => ({ flex:1, height:1, background: done ? '#4a7055' : '#ddd8cc', margin:'0 8px' }),
    // Layout
    layout: { display:'grid', gridTemplateColumns:'1fr 340px', gap:'2rem', maxWidth:1000, margin:'0 auto', padding:'2rem' },
    // Form
    formBox: { background:'#f4f1eb' },
    ...
    // Payment
    payMethods: { display:'flex', flexDirection:'column', gap:'0.75rem', marginBottom:'1.5rem' },
    payCard: (active) => ({ border:`1px solid ${active ? '#4a7055' : '#ddd8cc'}`, borderRadius:4, padding:'1rem 1.25rem', cursor:'pointer', background: active ? 'rgba(74,112,85,0.04)' : '#f9f7f2', display:'flex', alignItems:'center', gap:12, transition:'all 0.2s' }),
    ...
    // Buttons
    btnPrimary: { width:'100%', background:'#4a7055', color:'#f4f1eb', fontSize:'0.8rem', letterSpacing:'1.5px', textTransform:'uppercase', padding:14, border:'none', borderRadius:2, cursor:'pointer', fontFamily:"'DM Sans',sans-serif", marginBottom:8, opacity:1 },
    ...
    // Order Summary
    summary: { background:'#edeae0', border:'1px solid #ddd8cc', borderRadius:4, padding:'1.5rem', position:'sticky', top:80, height:'fit-content' },
    ...
    sumTotalLabel: { fontSize:'0.8rem', textTransform:'uppercase', letterSpacing:'1px' },
    sumTotalAmt: { fontFamily:"'Cormorant Garamond',serif", fontSize:'1.5rem', fontWeight:500, color:'#4a7055' },
    // Success
    ...
    successIcon: { width:72, height:72, borderRadius:'50%', background:'rgba(74,112,85,0.1)', border:'2px solid #4a7055', display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, margin:'0 auto 1.5rem' },
    ...
    successBtnGhost: { background:'transparent', color:'#2a2a1e', fontSize:'0.75rem', letterSpacing:'1.5px', textTransform:'uppercase', padding:'12px 28px', border:'1px solid #2a2a1e', borderRadius:2, cursor:'pointer', textDecoration:'none' },
    footer: { background:'#1e1e14', padding:'1.5rem 2rem', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'1rem', marginTop:'2rem' },
  ...
  if (step === 3) return (
    <div style={s.page}>
      ...
      <div style={s.successPage}>
        ...
        <p style={s.successP}>Thank you, <strong>{form.name || 'Sacred Soul'}</strong>. Your HAMPAR Spirituals order has been received. We will dispatch it within 2 business days from Jhansi.</p>
        <div style={s.orderCard}>
          ...
          {[
            ...
            ['Delivery', `${form.city || 'Your city'}, ${form.state}`],
    ...
    <div style={s.page}>
      ...
      <div style={s.layout}>
        ...
        <div style={s.formBox}>
          ...
          {step === 1 && (
            <>
              ...
              <button style={s.btnPrimary} onClick={handleNext}>Continue to Payment →</button>
              <Link href="/cart" style={{...s.btnBack, display:'block', textAlign:'center', textDecoration:'none'}}>← Back to Cart</Link>
            </>
          )}
          ...
          {step === 2 && (
            <>
                ...
                {[
                  ...
                  <div key={m.id} style={s.payCard(payMethod===m.id)} onClick={() => setPayMethod(m.id)}>
                    ...
                    <div>
                      ...
                      <div style={s.payNote}>{m.note}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {payMethod === 'cod' && (
                <div style={{marginBottom:'1.5rem',padding:'1.25rem',background:'#fff9ec',borderRadius:4,border:'1px solid #e8d8a0'}}>
                  <div style={{fontSize:'0.82rem',color:'#6a5a2a',lineHeight:1.8}}>
                    💡 <strong>COD Note:</strong> An additional ₹20 fee applies for Cash on Delivery. Please keep exact change ready. Our delivery partner will collect payment at your doorstep.
                  </div>
                </div>
              )}
              ...
              {payMethod === 'netbanking' && (
                <div style={{marginBottom:'1.5rem',padding:'1.25rem',background:'#edeae0',borderRadius:4,border:'1px solid #ddd8cc'}}>
                  ...
                  <select style={{...s.select,marginTop:6}}>
                    {['SBI','HDFC Bank','ICICI Bank','Axis Bank','Punjab National Bank','Bank of Baroda','Canara Bank','Union Bank','Other Bank'].map(b=><option key={b}>{b}</option>)}
                  </select>
                </div>
              )}
              ...
              <div style={{padding:'1rem 1.25rem',background:'#f0f5f0',border:'1px solid rgba(74,112,85,0.2)',borderRadius:4,marginBottom:'1.5rem'}}>
                ...
                <div style={{fontSize:'0.85rem',color:'#2a2a1e',lineHeight:1.7}}>{form.name} · {form.phone}</div>
                <div style={{fontSize:'0.82rem',color:'#6a6a52',lineHeight:1.7}}>{form.address}{form.landmark ? `, ${form.landmark}` : ''}, {form.city}, {form.state} — {form.pincode}</div>
              ...
              <button style={{...s.btnPrimary, opacity: placing ? 0.7 : 1}} onClick={handlePlaceOrder} disabled={placing}>
                {placing ? '⏳ Placing Order...' : `Place Order — ₹${total + (payMethod==='cod'?20:0)} →`}
              </button>
        ...
        <div style={s.summary}>
          ...
          <div style={s.sumRow}>
            <span>Shipping</span>
            <span style={{color: shipping===0 ? '#4a7055' : '#2a2a1e'}}>{shipping===0 ? 'FREE' : `₹${shipping}`}</span>
          </div>
          {payMethod === 'cod' && step === 2 && <div style={s.sumRow}><span>COD Fee</span><span>₹20</span></div>}
          <div style={s.sumDiv}/>
          <div style={s.sumTotal}>
            <span style={s.sumTotalLabel}>Total</span>
            <span style={s.sumTotalAmt}>₹{total + (payMethod==='cod' && step===2 ? 20 : 0)}</span>
          </div>
          <div style={{marginTop:'1.25rem',padding:'0.75rem',background:'rgba(74,112,85,0.06)',borderRadius:3,border:'1px solid rgba(74,112,85,0.15)'}}>
            ...
            {['100% Natural & Charcoal-Free','QR Purity Verified','Handcrafted in Jhansi'].map(t=>(
              <div key={t} style={{fontSize:'0.74rem',color:'#4a3d2c',padding:'2px 0',display:'flex',gap:6,alignItems:'center'}}>
                <span style={{color:'#4a7055'}}>✓</span>{t}
              </div>
            ))}
          </div>
        </div>
      ...
      <footer style={s.footer}>
        ...
        <div style={{display:'flex',gap:'1rem',flexWrap:'wrap'}}>
          {[['Privacy','/'],['Refund Policy','/'],['Contact','/']].map(([l,h])=>(
