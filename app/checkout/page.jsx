...
export default function CheckoutPage() {
  ...
  const s = {
    ...
    successBtn: { background:'#4a7055', color:'#f4f1eb', fontSize:'0.75rem', letterSpacing:'1.5px', textTransform:'uppercase', padding:'12px 28px', border:'none', borderRadius:2, textDecoration:'none' },
  ...
  if (!mounted) return <div style={{minHeight:'100vh',background:'#f4f1eb'}}/>
  ...
  if (step === 3) return (
    <div style={s.page}>
      ...
      <div style={s.successPage}>
        ...
        <p style={s.successP}>Thank you, <strong>{form.name}</strong>! Confirmation email sent to <strong>{form.email}</strong>. We dispatch within 2 business days from Jhansi.</p>
        <div style={s.orderCard}>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1rem',marginBottom:'0.75rem',color:'#2a2a1e'}}>Order Details</div>
          {[
            ['Order ID', orderId],
            ['Items', `${cart.reduce((s,i)=>s+i.qty,0)} items`],
            ['Total', `₹${total}`],
        ...
        <div style={s.successBtns}>
          ...
          <Link href="/purity" style={s.successBtnGhost}>Our Purity Promise</Link>
        </div>
      </div>
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.layout}>
        <div>
          {step===1 && <>
            ...
            <div style={s.formGroup}>
              ...
              <input style={s.input(false)} placeholder="Near temple..." value={form.landmark} onChange={e=>setForm({...form,landmark:e.target.value})}/>
            </div>
            <div style={s.formGrid2}>
              <div>
          ...
          {step===2 && <>
            ...
            {payMethod==='cod' && (
              <div style={{marginBottom:'1.5rem',padding:'1.25rem',background:'#fff9ec',borderRadius:4,border:'1px solid #e8d8a0'}}>
                <div style={{fontSize:'0.82rem',color:'#6a5a2a',lineHeight:1.8}}>💡 <strong>COD Note:</strong> ₹20 additional fee. Keep exact change ready.</div>
              </div>
            )}
            {payMethod==='netbanking' && (
              <div style={{marginBottom:'1.5rem',padding:'1.25rem',background:'#edeae0',borderRadius:4,border:'1px solid #ddd8cc'}}>
                ...
                <select style={{...s.select,marginTop:6}}>
                  {['SBI','HDFC Bank','ICICI Bank','Axis Bank','Punjab National Bank','Other'].map(b=><option key={b}>{b}</option>)}
                </select>
              </div>
            )}
        ...
        <div style={s.summary}>
          <div style={s.sumTitle}>Order Summary ({cart.reduce((s,i)=>s+i.qty,0)} items)</div>
          {cart.map((item,i) => (
          ...
          <div style={{marginTop:'1.25rem',padding:'0.75rem',background:'rgba(74,112,85,0.06)',borderRadius:3,border:'1px solid rgba(74,112,85,0.15)'}}>
            {['100% Natural & Charcoal-Free','QR Purity Verified','Handcrafted in Jhansi','📧 Email confirmation'].map(t=>(
